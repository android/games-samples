...
static void _printShaderLog(GLuint shader) {
    ...
    ALOGE("*** Getting info log for shader %u", shader);
...
static void _printProgramLog(GLuint program) {
    char buf[2048];
    memset(buf, 0, sizeof(buf));
    ALOGE("*** Getting info log for program %u", program);
...
void Shader::BindShader() {
    if (mProgramH == 0) {
        ALOGW("!!! WARNING: attempt to use shader before compiling.");
        ALOGW("!!! Compiling now. Shader: %s", GetShaderName());
        Compile();
    }
    glUseProgram(mProgramH);
}
...
void TrivialShader::Compile() {
    ...
    if (mColorLoc < 0) {
        ALOGE("*** Couldn't get color attrib location from shader.");
        ABORT_GAME;
    }
    mTintLoc = glGetUniformLocation(mProgramH, "u_Tint");
    if (mTintLoc < 0) {
        ALOGE("*** Couldn't get tint uniform location from shader.");
        ABORT_GAME;
    }
    UnbindShader();
}
