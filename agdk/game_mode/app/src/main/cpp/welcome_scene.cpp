...
void WelcomeScene::OnStartGraphics() {
    ...
    GameModeManager::getInstance().SetGameState(false, GAME_STATE_GAMEPLAY_INTERRUPTIBLE);
}
...
void WelcomeScene::RenderPanel() {
    ...
    ImGui::SetCursorPos(ImVec2((ImGui::GetWindowSize().x - text_size.x) * 0.5f, ImGui::GetWindowSize().y * 0.5f));
